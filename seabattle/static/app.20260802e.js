...
function renderRingball(mount, s){
  ...
  const BETWEEN_PAUSE = 0.7; // пауза перед следующим броском после попадания
  const MAX_TURN = 0.85; // макс. смена направления (~49°) — не прыгает на другую сторону
  ...
  function armNextBall(prevDir, pauseSec){
    ball = spawnBall(prevDir);
    ball.x = cx;
    ball.y = cy;
    ball.trail = [];
    countdown = pauseSec;
  ...
  function onScore(){
    ...
    setHud(score, best);
    // пауза + направление рядом с прошлым — не улетает сразу на другую сторону
    armNextBall(prev, BETWEEN_PAUSE);
    setHint('Дальше… крути кольцо');
...
async function startGame({vsAi=false, vsLocalMode=false}={}){
  ...
  // уже в своём лобби — не создаём вторую комнату (из‑за этого «прыгал» код)
  if(!vsAi && !vsLocalMode && code && token && state && state.phase==='lobby' && state.is_host){
