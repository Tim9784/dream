...
function applyState(s, opts={}){
  ...
  if(s.phase==='lobby'){
    ...
    // не оставляем на экране старую API-ошибку рематча
    if($('doneStatus') && /не окончена/i.test($('doneStatus').textContent||'')){
      // текст результата перезапишется ниже из s.message
    }
...
async function startFreshReplay(){
  // новая комната с теми же настройками — без гонок rematch/poll
  stopPoll();
...
$('btnReplay').onclick = async ()=>{
  ...
  if($('doneStatus')) $('doneStatus').textContent = 'Запускаем новую партию…';
  try{
    const solo = !!(state && (state.vs_ai || state.vs_local));
    // соло/локально и «залипший» экран — всегда новая комната
    if(solo || !state || !code || !token){
      await startFreshReplay();
      return;
    }
    // сеть: голос за рематч в той же комнате
    try{
      ...
      if(data.state){
        ...
        startPoll();
        // если UI всё ещё на done — создаём новую
        if(state && state.phase === 'done' && (data.restarted || data.already_started)){
          await startFreshReplay();
        }
        return;
      }
