const GAME_ICONS = {
  ...
  blik: `<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><ellipse cx="12" cy="12" rx="9.6" ry="6.4" transform="rotate(-35 12 12)" stroke="currentColor" stroke-width="2.2"/><ellipse cx="12" cy="12" rx="4.2" ry="2.7" transform="rotate(-35 12 12)" stroke="currentColor" stroke-width="1.6" opacity=".55"/></svg>`,
  ...
  ringball: `<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="8.2" stroke="currentColor" stroke-width="2.2" stroke-dasharray="38 14" stroke-linecap="round" transform="rotate(-40 12 12)"/><circle cx="12" cy="9.2" r="1.7" fill="currentColor" stroke="none"/></svg>`,
};
...
const GAMES = {
  ...
  hangman: {title:'Виселица', blurb:'Загадай слово или отгадай по буквам'},
  ringball: {title:'Кольцо', blurb:'Крути отверстие — лови шарик. Только соло', solo_only:true, no_rating:true},
};
...
const GAME_UTM = {
  ...
  ringball:   {campaign:'ringball',   content:'kolco'},
};
...
function trackGameUtm(gameId, action){
  ...
  // UTM только для Метрики — НЕ пишем в адресную строку, иначе люди копируют
  // ссылку с utm_campaign=seabattle без ?join= и «рандомно» попадают не туда.
  let hitPath = location.pathname || '/';
  try{
...
// защита от устаревшего poll после «В меню» / выхода
let sessionGen = 0;
...
function isSoloGame(gameId){
  const meta = GAMES[gameId || chosenGame];
  return !!(meta && meta.solo_only);
}

function openSetup(gameId){
  chosenGame = gameId || chosenGame;
  const solo = isSoloGame(chosenGame);
  ...
  if($('btnCreate')) $('btnCreate').classList.toggle('hidden', solo);
  if($('btnLocal')) $('btnLocal').classList.toggle('hidden', solo);
  if($('btnVsAi')) $('btnVsAi').classList.toggle('hidden', solo);
  if($('btnSoloPlay')) $('btnSoloPlay').classList.toggle('hidden', !solo);
  if($('localHint')) $('localHint').classList.toggle('hidden', solo);
  if($('soloHint')) $('soloHint').classList.toggle('hidden', !solo);
  if($('setupJoinBlock')) $('setupJoinBlock').classList.toggle('hidden', solo);
...
async function api(path, opts={}){
  let url = path;
  // привилегия Тимофея: peek=1 только если включено на клиенте
  if(typeof durakPeekEnabled === 'function' && durakPeekEnabled()){
    if(url.indexOf('/api/room/') === 0 && url.indexOf('peek=') === -1){
      url += (url.indexOf('?') >= 0 ? '&' : '?') + 'peek=1';
    }
  }
  let body = opts.body;
  if(body && typeof durakPeekEnabled === 'function' && durakPeekEnabled()){
    try{
      const parsed = typeof body === 'string' ? JSON.parse(body) : body;
      if(parsed && typeof parsed === 'object' && parsed.peek == null){
        parsed.peek = true;
  ...
  const res = await fetch(url, {
    ...
    ...opts,
    body: body !== undefined ? body : opts.body
  });
...
function canPeekCardsAccount(){
  return !!(currentUser && currentUser.can_peek_cards);
}

function durakPeekEnabled(){
  if(!canPeekCardsAccount()) return false;
  try{ return localStorage.getItem('omove-durak-peek') === '1'; }catch(_){ return false; }
}

function setDurakPeekEnabled(on){
  try{ localStorage.setItem('omove-durak-peek', on ? '1' : '0'); }catch(_){}
  lastPlaySig = '';
  if(state && state.game==='durak' && code && token){
    pollRoomOnce().catch(()=>{});
  }
}
...
function renderAccount(){
  ...
  if(currentUser){
    box.innerHTML = `
      <button type="button" class="account-chip account-chip-btn" id="btnProfile" title="Профиль · ${escapeHtml(currentUser.email || '')}">
...
async function openProfile(){
  ...
  const m = $('profileModal');
  if(!m) return;
  const list = $('profileGames');
  const totals = $('profileTotals');
  const nameEl = $('profileName');
  const emailEl = $('profileEmail');
  if(nameEl) nameEl.textContent = currentUser.name || 'Игрок';
  if(emailEl) emailEl.textContent = currentUser.email || '';
  if(totals) totals.innerHTML = '<span>Загрузка…</span>';
  ...
  try{
    ...
    const tw = Number((data.totals && data.totals.wins) || currentUser.wins || 0);
    const tg = Number((data.totals && data.totals.games) || currentUser.games || 0);
    if(totals){
      totals.innerHTML = `<span><b>${tg}</b> партий</span><span><b>${tw}</b> побед</span>`;
    }
    if(nameEl) nameEl.textContent = (data.user && data.user.name) || currentUser.name || 'Игрок';
    if(emailEl) emailEl.textContent = (data.user && data.user.email) || currentUser.email || '';
    const peekBox = $('profilePeekBox');
    const peekToggle = $('profilePeekToggle');
    if(peekBox && peekToggle){
      const allow = !!(currentUser && currentUser.can_peek_cards);
      peekBox.classList.toggle('hidden', !allow);
      if(allow){
        peekToggle.checked = durakPeekEnabled();
        peekToggle.onchange = ()=> setDurakPeekEnabled(!!peekToggle.checked);
      }
    }
    ...
    if(list){
      ...
      if(!played.length){
        list.innerHTML = `<li class="profile-empty">Пока нет учтённых партий. Сыграй по сети или с роботом — статистика появится здесь.</li>`;
      } else {
        list.innerHTML = played.map(r => {
          const icon = GAME_ICONS[r.game] || '';
          return `<li class="profile-game">
            <span class="profile-game-mark" aria-hidden="true">${icon}</span>
            <span class="profile-game-body">
              <strong>${escapeHtml(r.title || r.game)}</strong>
              <small>${Number(r.games)||0} парт. · ${Number(r.wins)||0} побед</small>
            </span>
            <span class="profile-game-wins" title="Победы">${Number(r.wins)||0}</span>
          </li>`;
        }).join('');
      }
    }
    ...
    if(totals) totals.innerHTML = `<span>${escapeHtml(e.message || 'Не удалось загрузить профиль')}</span>`;
  }
}
...
function openLogoutModal(){
  ...
  m.classList.remove('hidden');
  m.setAttribute('aria-hidden', 'false');
  if($('btnLogoutNo')) $('btnLogoutNo').focus();
  return true;
}
...
function extractAuthToken(raw){
  ...
  // старые ссылки/hex на всякий случай
...
async function sendAuthLink(){
  ...
  try{
    ...
    let msg = `Код отправлен на ${data.email || email}. Вставь его ниже и нажми «Войти по коду».`;
...
async function loginWithAuthCode(){
  ...
  if(!token){
    $('authErr').textContent = 'Вставь 6-значный код из письма';
    return;
  }
  try{
    ...
    if(currentUser){
      closeAuthModal();
      if($('homeErr')) $('homeErr').textContent = `Вы вошли как ${currentUser.name}. Победы будут в рейтинге.`;
      if(state && state.phase==='done') updateDoneAuthHint(state);
    }
  }catch(e){
    $('authErr').textContent = e.message || 'Не удалось войти по коду';
  }
}
...
async function verifyAuthToken(token){
  try{
    const data = await api('/api/auth/verify', {
      method: 'POST',
      body: JSON.stringify({token, code: token}),
    });
    currentUser = data.user || null;
    renderAccount();
    loadRating();
    if($('homeErr') && currentUser){
      $('homeErr').textContent = `Вы вошли как ${currentUser.name}. Победы будут в рейтинге.`;
    }
  }catch(e){
    if($('authErr') && !$('authModal').classList.contains('hidden')){
      $('authErr').textContent = e.message || 'Код неверный или устарел';
    } else if($('homeErr')){
      $('homeErr').textContent = e.message || 'Код неверный или устарел';
    }
    throw e;
  }
  try{
...
if($('btnLogoutNo')) $('btnLogoutNo').onclick = closeLogoutModal;
if($('btnLogoutYes')) $('btnLogoutYes').onclick = logoutUser;
if($('logoutModal')){
  $('logoutModal').addEventListener('click', (e)=>{
    if(e.target === $('logoutModal')) closeLogoutModal();
  });
}
if($('btnProfileClose')) $('btnProfileClose').onclick = closeProfileModal;
if($('profileModal')){
...
const POLL_MS_LOBBY = 850;
const POLL_MS_PLAY = 380;

function pollDelayMs(){
  if(state && (state.phase==='playing' || state.phase==='placing')) return POLL_MS_PLAY;
...
async function pollRoomOnce(){
  ...
  try{
    const data = await api(`/api/room/${roomCode}?token=${encodeURIComponent(roomToken)}`);
    if(gen !== sessionGen || !code || !token) return;
    roomMisses = 0;
    const fromDone = !!(state && state.phase==='done' && data.state && data.state.phase!=='done');
    applyState(data.state, {force: !!fromDone, sessionGen: gen});
      ...
      if(roomMisses >= 4){
        try{
          ...
          const data = await api('/api/room/join', {
            method:'POST',
            body: JSON.stringify({ name: joinName, code: roomCode, token: roomToken })
          });
          if(gen !== sessionGen) return;
          token = data.token; code = data.code;
          roomMisses = 0;
          LS.set({ ...(saved||{}), token, code, name: joinName, game: (data.state&&data.state.game)||chosenGame });
          applyState(data.state, {sessionGen: gen});
          return;
        }catch(_){}
        if(gen === sessionGen) goHome('Комната закрыта');
      }
    }
  }
}
...
function schedulePoll(){
  ...
  const loop = async ()=>{
    await pollRoomOnce();
    if(code && token) pollTimer = setTimeout(loop, pollDelayMs());
  };
  pollTimer = setTimeout(loop, 0);
}

function startPoll(){ schedulePoll(); }
...
function needsPrivacy(game){
  // экран «я за экраном» — морской бой, карты и виселица (слово / буквы)
  return game==='seabattle' || game==='durak' || game==='blik' || game==='hangman';
}
...
function desiredHotseatSlot(s){
  ...
  if(s.game==='hangman'){
    const gs = s.game_state || {};
    if(gs.round_phase==='pick_word' && gs.setter) return gs.setter;
    if(gs.guesser) return gs.guesser;
    return s.turn || 'p1';
  }
  if(s.phase==='placing'){
...
function renderHandover(s, nextSlot){
  ...
  const gs = s.game_state || {};
  let hint = 'Пусть второй игрок отвернётся — сейчас откроется твоё поле.';
  if(s.game==='durak' || s.game==='blik'){
    hint = 'Пусть второй игрок отвернётся — сейчас откроются твои карты.';
  } else if(s.game==='hangman'){
    hint = gs.round_phase==='pick_word'
      ? 'Пусть отгадывающий отвернётся — сейчас можно загадать слово.'
      : 'Пусть загадавший отвернётся — сейчас откроются буквы для отгадки.';
  } else if(s.game==='seabattle'){
    hint = 'Пусть второй игрок отвернётся — сейчас откроется твоё поле с кораблями.';
  }
  mount.innerHTML = `
    <div class="handover">
...
function applyState(s, opts={}){
  if(!s) return;
  // после выхода в меню устаревший ответ poll не должен вернуть экран игры
  if(opts.sessionGen != null && opts.sessionGen !== sessionGen) return;
  // без активной комнаты не показываем lobby/playing/done (force тоже нельзя)
  if(!code || !token) return;
  ...
  if(s.phase==='lobby'){
    ...
    if(s.code){
      // не даём коду «прыгать»: фиксируем первый валидный код сессии
      if(!code || code === s.code){
        code = s.code;
        $('codeView').textContent = s.code;
      } else if($('codeView') && !$('codeView').textContent.replace(/\D/g,'')){
    ...
    const statusLine = s.game==='durak'
      ? durakPlayStatus(s)
      : (who + (s.message || ''));
    setPlayStatus(s, statusLine);
    setWinChance(s);
    if(!pollTimer) startPoll();
    ...
    if(enteredDone){
      refreshMe().then(()=> updateDoneAuthHint(s));
      loadRating();
    } else {
      updateDoneAuthHint(s);
    }
    ...
    if(rematchBtn && !rematchBusy){
      if(s.vs_ai || s.vs_local || s.solo_only || isSoloGame(s.game)){
...
function updateDoneAuthHint(s){
  const el = $('doneAuthHint');
  if(!el) return;
  // «вдвоём здесь» / соло без рейтинга — не зовём регистрироваться; для вошедших тоже не показываем
  if((s && (s.vs_local || s.no_rating || s.solo_only || isSoloGame(s.game))) || currentUser){
  ...
  el.innerHTML = `
    <p>Зарегистрируйтесь, чтобы мы записали вашу игру в рейтинг.</p>
    <button type="button" class="btn ghost" id="btnDoneRegister">Войти</button>
  `;
  const btn = $('btnDoneRegister');
  if(btn) btn.onclick = openAuthModal;
}
...
async function doAction(payload){
  ...
  try{
    ...
    if(gen !== sessionGen || !code || !token) return;
    applyState(data.state, {force:true, sessionGen: gen});
    // догоняем ответ соперника/робота без ожидания следующего poll
    setTimeout(()=>{ if(gen === sessionGen) pollRoomOnce().catch(()=>{}); }, 120);
  }catch(e){ if(gen === sessionGen) $('playErr').textContent=e.message; }
}
...
function renderGame(s){
  ...
  if(s.game==='ringball') return renderRingball(mount, s);
}
...
function renderHangman(mount, s){
  ...
  const roundPhase = gs.round_phase || gs.phase || 'playing';
  const setterName = (gs.setter && s.players && s.players[gs.setter] && s.players[gs.setter].name) || '';
  const guesserName = (gs.guesser && s.players && s.players[gs.guesser] && s.players[gs.guesser].name) || '';
  const parts = [];
  ...
  if(wrong>0) parts.push(`<circle cx="90" cy="50" r="12"/>`);
  if(wrong>1) parts.push(`<line x1="90" y1="62" x2="90" y2="96"/>`);
  if(wrong>2) parts.push(`<line x1="90" y1="70" x2="76" y2="84"/>`);
  if(wrong>3) parts.push(`<line x1="90" y1="70" x2="104" y2="84"/>`);
  if(wrong>4) parts.push(`<line x1="90" y1="96" x2="80" y2="118"/>`);
  if(wrong>5) parts.push(`<line x1="90" y1="96" x2="100" y2="118"/>`);

  let bodyHtml = '';
  if(roundPhase==='pick_word'){
    if(gs.can_set_word){
      bodyHtml = `
        <div class="hangman-pick">
          <div class="hangman-role">Ты загадываешь слово${guesserName? ' для '+escapeHtml(guesserName):''}</div>
          <label class="hangman-word-label"><span>Слово (2–16 букв)</span>
            <input id="hgWord" type="text" maxlength="16" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="НАПРИМЕР">
          </label>
          <button type="button" class="btn" id="hgSetWord">Загадать</button>
          <p class="hint">Только кириллица, без пробелов. Второй игрок не должен видеть экран.</p>
        </div>`;
    } else {
      bodyHtml = `
        <div class="hangman-pick">
          <div class="hangman-role">${escapeHtml(setterName||'Соперник')} загадывает слово…</div>
          <p class="hint">Отвернись от экрана — скоро твоя очередь угадывать.</p>
        </div>`;
    }
  } else {
    const secret = gs.word ? `<div class="hangman-secret">Слово: <strong>${escapeHtml(gs.word)}</strong></div>` : '';
    const role = gs.mode==='versus'
      ? `<div class="hangman-role">${gs.can_guess ? 'Ты отгадываешь' : (setterName ? escapeHtml(setterName)+' смотрит · отгадывает '+(guesserName||'соперник') : 'Идёт отгадка')}</div>`
      : '';
    bodyHtml = `
      <div class="hangman-top">
        ...
        <div class="hangman-info">
          ${role}
          <div class="hangman-word" aria-live="polite">${escapeHtml(masked)}</div>
  ...
  mount.innerHTML = `<div class="hangman">${bodyHtml}</div>`;

  if(roundPhase==='pick_word' && gs.can_set_word){
    const input = $('hgWord');
    const btn = $('hgSetWord');
    const submit = ()=>{
      const word = (input && input.value || '').trim();
      doAction({type:'set_word', word});
    };
    ...
    if(input){
      ...
      input.addEventListener('input', ()=>{
        input.value = input.value.toUpperCase().replace(/[^А-ЯЁа-яё]/g,'').slice(0,16);
      });
    }
    return;
  }
  ...
  letters.forEach(L=>{
    ...
    b.textContent=L;
    if(guessed.has(L) || s.phase==='done' || !gs.can_guess){ b.disabled=true; }
...
/* ===== Ringball (Кольцо) ===== */
let ringballLive = null;

function stopRingball(){
  if(!ringballLive) return;
  try{ ringballLive.stop(); }catch(_){}
  ringballLive = null;
}

function renderRingball(mount, s){
  const gs = s.game_state || {};
  const serverScore = Number(gs.score) || 0;
  const serverBest = Number(gs.best) || 0;
  const playing = s.phase === 'playing';

  if(
    ringballLive &&
    ringballLive.mount === mount &&
    ringballLive.code === (s.code || code) &&
    ringballLive.playing === playing
  ){
    ringballLive.setHud(serverScore, serverBest);
    return;
  }
  ...
  mount.innerHTML = `
    <div class="ringball">
      <div class="ringball-hud">
        <div class="ringball-score" id="rbScore">Счёт: ${serverScore}</div>
        <div class="ringball-best" id="rbBest">Рекорд: ${serverBest}</div>
      </div>
      <canvas class="ringball-canvas" id="rbCanvas" width="440" height="440" aria-label="Кольцо с отверстием"></canvas>
      <p class="ringball-hint">Крути кольцо — шарик должен выпасть в отверстие</p>
    </div>`;

  const canvas = $('rbCanvas');
  const scoreEl = $('rbScore');
  const bestEl = $('rbBest');
  if(!canvas || !playing){
    ringballLive = {
      mount, code: s.code || code, playing: false,
      setHud(sc, be){
        if(scoreEl) scoreEl.textContent = 'Счёт: ' + sc;
        if(bestEl) bestEl.textContent = 'Рекорд: ' + Math.max(be, sc);
      },
      stop(){},
    };
    return;
  ...
  const RING_W = 22;
  const BALL_R = 14;
  const GAP = 1.15; // радианы отверстия (~66°)
  let ringAngle = -Math.PI / 2; // центр отверстия
  let score = serverScore;
  let best = Math.max(serverBest, score);
  let finished = false;
  let raf = 0;
  let lastTs = 0;
  let keys = {left:false, right:false};
  let drag = null;
  // отсчёт перед первым броском — успеть навести отверстие
  let countdown = 3; // 3 → 2 → 1 → 0 (старт); после очка — короткая пауза (дробь)
  let countdownAcc = 0;
  const COUNTDOWN_STEP = 0.85;
  const MAX_TURN = Math.PI * 2 / 3; // макс. смена направления — 120°
  ...
  function pickDir(prev){
    if(prev == null || !Number.isFinite(prev)){
      return Math.random() * Math.PI * 2;
    }
    // небольшое отклонение от прошлого курса — без разворота на 180°
    const delta = (Math.random() * 2 - 1) * MAX_TURN;
    return prev + delta;
  }

  function spawnBall(prevDir){
    const a = pickDir(prevDir);
    lastDir = a;
    const spd = ballSpeed();
    return {
      x: cx + Math.cos(a) * 22,
  ...
  let ball = spawnBall(null);
  // во время отсчёта шарик стоит в центре, направление уже выбрано
  ball.x = cx;
  ball.y = cy;
  ...
  setHint('Приготовься… крути кольцо');

  function setHud(sc, be){
    if(typeof sc === 'number') score = Math.max(score, sc);
    if(typeof be === 'number') best = Math.max(best, be);
    if(scoreEl) scoreEl.textContent = 'Счёт: ' + score;
    if(bestEl) bestEl.textContent = 'Рекорд: ' + Math.max(best, score);
  }
  ...
  function inGap(ballAngle){
    return Math.abs(normAngle(ballAngle - ringAngle)) <= GAP / 2;
  }

  function themeColors(){
    const light = document.documentElement.getAttribute('data-theme') === 'light';
    return light
      ? {ring:'#18181b', gapBg:'#f0f0f2', ball:'#18181b', accent:'#3f3f46', soft:'rgba(24,24,27,.18)'}
      : {ring:'#fafafa', gapBg:'#121214', ball:'#fafafa', accent:'#d4d4d8', soft:'rgba(250,250,250,.14)'};
  }
  ...
  function draw(){
    ...
    // soft orbit guide
    ctx.beginPath();
    ctx.arc(cx, cy, R - RING_W * 0.35, 0, Math.PI * 2);
    ...
    // ring with gap
    ctx.beginPath();
    ctx.arc(cx, cy, R, ringAngle + GAP / 2, ringAngle - GAP / 2 + Math.PI * 2, false);
    ...
    for(const side of [-1, 1]){
      const a = ringAngle + side * GAP / 2;
      const x = cx + Math.cos(a) * R;
      const y = cy + Math.sin(a) * R;
      ctx.beginPath();
      ctx.arc(x, y, RING_W * 0.38, 0, Math.PI * 2);
    ...
    for(let i = 0; i < ball.trail.length; i++){
      ...
      ctx.beginPath();
      ctx.arc(t.x, t.y, BALL_R * (0.35 + 0.45 * k), 0, Math.PI * 2);
      ctx.fillStyle = lightAlpha(c.ball, 0.12 + 0.35 * k);
      ctx.fill();
    }
    ...
    // направление полёта во время паузы / отсчёта
    if(countdown > 0){
      const spd = Math.hypot(ball.vx, ball.vy) || 1;
      const ux = ball.vx / spd;
      const uy = ball.vy / spd;
      const tip = R - RING_W * 0.9;
      ctx.beginPath();
      ctx.moveTo(cx + ux * (BALL_R + 10), cy + uy * (BALL_R + 10));
      ctx.lineTo(cx + ux * tip, cy + uy * tip);
      ctx.strokeStyle = lightAlpha(c.accent, 0.45);
      ctx.lineWidth = 3;
      ctx.setLineDash([8, 8]);
      ctx.stroke();
      ctx.setLineDash([]);

      // цифры только на стартовом 3–2–1, между очками — просто стрелка
      if(countdown >= 1){
        const label = String(Math.ceil(countdown));
        ctx.save();
        ctx.font = "800 72px Syne, sans-serif";
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillStyle = lightAlpha(c.ring, 0.9);
  ...
  function finishMiss(){
    if(finished) return;
    finished = true;
    doAction({type:'miss', score});
  }

  function onScore(){
    const prev = (ball && Number.isFinite(ball.dir)) ? ball.dir : lastDir;
    score += 1;
    if(score > best) best = score;
    setHud(score, best);
    // сразу новый бросок, но курс рядом с прошлым — без паузы и стрелки
    ball = spawnBall(prev);
    countdown = 0;
    doAction({type:'score', score});
  }
  ...
  function onKey(e, down){
    if(e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A' || e.key === 'ф' || e.key === 'Ф'){
      keys.left = down; e.preventDefault();
    }
    if(e.key === 'ArrowRight' || e.key === 'd' || e.key === 'D' || e.key === 'в' || e.key === 'В'){
      keys.right = down; e.preventDefault();
    }
  }

  function onPointerDown(e){
    drag = {startAng: pointerAngle(e.clientX, e.clientY), ring0: ringAngle};
    try{ canvas.setPointerCapture(e.pointerId); }catch(_){}
  }
  function onPointerMove(e){
    if(drag == null) return;
    const ang = pointerAngle(e.clientX, e.clientY);
    ringAngle = drag.ring0 + (ang - drag.startAng);
  }
  function onPointerUp(){ drag = null; }

  function tick(ts){
    if(finished || !ringballLive) return;
    if(!lastTs) lastTs = ts;
    let dt = Math.min(0.032, (ts - lastTs) / 1000);
    lastTs = ts;

    const rot = 2.8;
    if(keys.left) ringAngle -= rot * dt;
    if(keys.right) ringAngle += rot * dt;

    if(countdown > 0){
      const startCount = countdown >= 1; // режим 3–2–1 vs короткая пауза между очками
      if(startCount){
        countdownAcc += dt;
        while(countdown > 0 && countdownAcc >= COUNTDOWN_STEP){
          countdownAcc -= COUNTDOWN_STEP;
          countdown -= 1;
        }
      } else {
        countdown -= dt;
      ...
      if(countdown <= 0){
        countdown = 0;
        // отпускаем шарик из центра по уже выбранному направлению
        const spd = Math.hypot(ball.vx, ball.vy) || ballSpeed();
        const ux = ball.vx / (Math.hypot(ball.vx, ball.vy) || 1);
        const uy = ball.vy / (Math.hypot(ball.vx, ball.vy) || 1);
        ball.x = cx + ux * 22;
        ball.y = cy + uy * 22;
        ball.vx = ux * spd;
        ball.vy = uy * spd;
        setHint('Крути кольцо — шарик должен выпасть в отверстие');
      } else if(startCount){
        setHint('Приготовься… ' + Math.ceil(countdown));
      }
      draw();
    ...
    const hitAt = R - RING_W * 0.35 - BALL_R;
    ...
    if(dist >= hitAt){
      ...
      if(inGap(ang)){
        ...
        // snap to rim for last frame, then end
        const k = hitAt / Math.max(dist, 0.001);
        ball.x = cx + dx * k;
        ball.y = cy + dy * k;
        draw();
        finishMiss();
...
function durakPlayStatus(s){
  const gs = s.game_state || {};
  const attSlot = gs.attacker;
  const defSlot = gs.defender;
  const att = attSlot && s.players && s.players[attSlot] && s.players[attSlot].name;
  const def = defSlot && s.players && s.players[defSlot] && s.players[defSlot].name;
  const parts = [];
  if(att) parts.push('Атака: '+att);
  if(def) parts.push('Кладут на: '+def);
  const expect = {
    attack:'ход атакующего',
    defend:'отбивает защитник',
    throw:'подкидывают'
  }[gs.expect];
  if(expect) parts.push(expect);
...
function renderDurak(mount, s){
  ...
  const peekOn = !!(gs.peek || s.peek_cards);
  const canPeek = !!(gs.can_peek || s.can_peek_cards || canPeekCardsAccount());
  const peekBtn = canPeek
    ? `<button type="button" class="btn ghost peek-btn${peekOn?' on':''}" id="btnDurakPeek">${peekOn?'Скрыть карты':'Карты соперников'}</button>`
    : '';
  const canAdmin = !!(gs.can_admin_tools || canPeek);
  const trumpLeft = (gs.trump_left != null) ? Number(gs.trump_left) : null;
  const nextTrumpOn = !!gs.next_trump_armed;
  const adminHtml = canAdmin
    ? `<div class="durak-admin">
         <span class="durak-admin-stat">Козырей в колоде: <strong>${trumpLeft==null?'—':trumpLeft}</strong></span>
         <button type="button" class="btn ghost peek-btn${nextTrumpOn?' on':''}" id="btnDurakNextTrump"${(trumpLeft===0 && !nextTrumpOn)?' disabled':''}>${nextTrumpOn?'Отменить след. козырь':'След. набор — козырь'}</button>
       </div>`
    : '';
  ...
  const peekToggle = $('btnDurakPeek');
  if(peekToggle){
    peekToggle.onclick = ()=>{
      setDurakPeekEnabled(!durakPeekEnabled());
    };
  }
  const nextTrumpBtn = $('btnDurakNextTrump');
  if(nextTrumpBtn){
    nextTrumpBtn.onclick = ()=>{
      doAction({type:'cheat_next_trump', on: !nextTrumpOn});
    };
  }

  mount.querySelectorAll('[data-backs]').forEach(box=>{
    const slot = box.getAttribute('data-backs');
    const hand = (gs.hands && gs.hands[slot]) || [];
    const n = (gs.hand_counts && gs.hand_counts[slot]) || hand.length || 0;
    const showFaces = peekOn && hand.some(c => !!c);
    if(showFaces){
      hand.forEach(code=>{
        if(!code) return;
  ...
  if(!table.length){
    ...
    table.forEach(pair=>{
      ...
      a.className = 'dcard attack'+(cardRed(pair.a)?' red':'');
...
async function leaveGame(){
  ...
  // сразу уходим в меню — не ждём сеть, иначе poll снова рисует «Игра окончена»
  goHome();
  try{
    if(wasCode && wasToken){
...
async function startGame({vsAi=false, vsLocalMode=false}={}){
  ...
  // уже в своём лобби ЭТОЙ же игры — не создаём вторую комнату (из‑за этого «прыгал» код)
  if(
    !vsAi && !vsLocalMode &&
    code && token && state && state.phase==='lobby' && state.is_host &&
    state.game === chosenGame
  ){
    applyState(state, {force:true});
    return { ok:true, code, token, state };
  }
  // лобби другой игры / другая сессия — сначала выходим, иначе «кидало» в старый морской бой
  if(code && token && state && state.game && state.game !== chosenGame){
    const wasCode = code, wasToken = token;
    stopPoll();
  ...
  try{
    ...
    lastFinishedSig = '';
    clearShareHint(true);
    if($('codeView') && data.code) $('codeView').textContent = data.code;
...
if($('btnSoloPlay')) $('btnSoloPlay').onclick = ()=>{ setLocalNamesVisible(false); startGame({vsAi:false, vsLocalMode:false}); };
$('btnLocal').onclick = ()=>{
...
$('btnReplay').onclick = async ()=>{
  ...
  const wantSolo = !!(state && (state.solo_only || isSoloGame(state.game))) || isSoloGame(chosenGame);
  if($('doneStatus')) $('doneStatus').textContent = 'Запускаем новую партию…';
  try{
    stopRingball();
    // 1) робот / вдвоём здесь / соло — сначала рематч в той же комнате (1 запрос)
    if(code && token){
      try{
        ...
        if(data.state && !wantAi && !wantLocal && !wantSolo){
          lastPlaySig = '';
          applyState(data.state, {force:true});
          startPoll();
          return;
    ...
    if(wantAi || wantLocal || wantSolo || !state){
    ...
    rematchBusy = false;
    if(rematchBtn && state && state.phase==='done' && !state.vs_ai && !state.vs_local && !state.solo_only && !isSoloGame(state.game)){
...
function inviteUrl(roomCode){
  ...
  // всегда чистый URL без utm_* из адресной строки
...
function readJoinCodeFromUrl(){
  try{
    ...
    if(c.length===6){
      // оставляем в URL только join до входа, потом чистим всё лишнее
      u.searchParams.delete('join');
      // убираем служебные utm, чтобы не путали при копировании
...
async function shareInvite(){
  const hint = $('shareHint');
  const fromView = ($('codeView') && $('codeView').textContent) || '';
  const roomCode = String(code || (state && state.code) || fromView).replace(/\D/g,'').slice(0,6);
  if(!roomCode || roomCode.length!==6){
    if(hint){ hint.classList.remove('ok'); hint.textContent = 'Код ещё не готов'; delete hint.dataset.code; }
    return;
  }
  // синхронизируем отображаемый код со ссылкой
  code = roomCode;
  if($('codeView')) $('codeView').textContent = roomCode;
  ...
  try{
    if(navigator.share){
      ...
      if(hint){ hint.classList.add('ok'); hint.textContent = 'Отправлено · код '+roomCode; hint.dataset.code = roomCode; }
      return;
    }
  }catch(e){
  ...
  try{
    await copyText(url);
    if(hint){ hint.classList.add('ok'); hint.textContent = 'Ссылка скопирована · код '+roomCode; hint.dataset.code = roomCode; }
  }catch(_){
...
(async function boot(){
  ...
  // чистим старые utm_* из адресной строки (раньше писались при выборе игры)
  try{
    const u = new URL(location.href);
    let dirty = false;
    ...
    if(dirty && !u.searchParams.get('join') && !u.searchParams.get('auth')){
      history.replaceState(history.state || {}, '', u.pathname + (u.search||'') + (u.hash||''));
    }
  }catch(_){}
  ...
  if(pendingJoin){
    ...
    try{
      ...
      // не открываем сетап случайной игры (по умолчанию seabattle) —
      // оставляем код в поле и домашний экран / сетап без смены игры
      if($('joinCode')) $('joinCode').value = pendingJoin;
      show('home');
      if($('homeErr')) $('homeErr').textContent = e.message || 'Не удалось войти по ссылке';
