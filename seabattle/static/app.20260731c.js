...
function updateDoneAuthHint(s){
  ...
  if(!el) return;
  // «вдвоём здесь» в рейтинг не идёт; для вошедших тоже не показываем
  if((s && s.vs_local) || currentUser){
  ...
  if(!table.length){
    ...
    table.forEach(pair=>{
      ...
      if(pair.d){
        ...
        const suitHint = document.createElement('div');
        suitHint.className = 'dpair-suit';
        suitHint.textContent = (DURAK_SUIT[pair.a[1]]||'');
        if(cardRed(pair.a)) suitHint.classList.add('red');
        wrap.appendChild(suitHint);
      }
